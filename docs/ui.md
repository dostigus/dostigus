...
fill, so the thread cannot show through that strip. A rectangular
`--bg-chat` plate, the full width of that inset and with no radius,
sits behind the row so the rounded corners rest on the Chat canvas. End padding
