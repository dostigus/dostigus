...
it('overlays a Chat pill and pads the thread above it', () => {
  ...
  expect(pill).toContain('padding: 0.14rem 0.1rem')
  expect(pill).toContain('--pill-slot: 0.62rem')
  expect(pill).not.toContain('width: 100%')
  expect(chat).toContain('class="cue-balance"')
  expect(chat).toContain('class="identity-copy"')
  expect(chat).not.toContain('padding: 0.2rem 0.7rem 0.2rem 0.2rem')
  const cue = block('.cue {', '.identity:hover:not(:disabled) .cue')
  expect(cue).toContain('opacity: 0')
  expect(cue).not.toContain('width: 0')
})
