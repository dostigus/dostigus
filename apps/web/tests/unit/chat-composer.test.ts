...
it('overlays a slim composer on one scrolling Chat pane', () => {
  ...
  const plate = block('.composer-plate {', '.composer-row,')
  expect(plate).toContain('background: var(--bg-chat)')
  expect(plate).toContain('padding-bottom: 2px')
  expect(plate).not.toContain('border-radius')
