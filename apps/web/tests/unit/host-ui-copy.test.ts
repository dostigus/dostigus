...
it('splits Dashboard into Overview, Cluster settings, Providers, and Settings', () => {
  const shell = read('pages/dashboard.vue')
  const layout = read('layouts/dashboard.vue')
  expect(shell).toContain('<NuxtPage />')
  expect(shell).toContain('layout: \'dashboard\'')
  expect(shell).toContain('dashboard.title')
  expect(shell).not.toContain('HostMenuButton')
  expect(shell).not.toContain('class="top"')
  ...
  expect(layout).toContain('dashboard.nav.providers')
  expect(layout).toContain('dashboard.nav.clusterSettings')
  expect(layout).toContain('dashboard.nav.members')
  expect(layout).toContain('dashboard.nav.back')
  expect(layout).toContain('dashboard.nav.backAria')
  expect(layout).toContain('to="/"')
  expect(layout).toContain('$t(\'dashboard.aria\')')
  ...
  const cluster = read('pages/dashboard/cluster.vue')
  expect(cluster).toContain('settings.other.timezone.title')
  expect(cluster).toContain('settings.other.allowlist.title')
  expect(cluster).toContain('settings.other.locale.title')
  expect(cluster).toContain('/api/settings/timezone')
  expect(cluster).toContain('/api/settings/http-allowlist')
  ...
  expect(tHost('ru', 'dashboard.nav.providers')).toBe('Providers')
  expect(tHost('ru', 'dashboard.nav.clusterSettings')).toBe('Настройки кластера')
  for (const page of [shell, layout, cluster]) {
...
it('leads Providers with Bots ↔ Providers, key-first OpenRouter, shelf, and health', () => {
  const providers = read('pages/dashboard/providers.vue')
...
it('keeps Members copy product-facing', () => {
  const members = read('pages/dashboard/members.vue')
